...
const creador = document.querySelector('#createdBy').innerHTML = 'Aplicación creada por:Andres';
//creador.innerHTML = 'Aplicación creada por:Andres'
...
function buildToDo(todo, index) {
  ...
  toDoText.id = index;
  document.body.appendChild(toDoShell);
  if (this.complete) {
...
function buildToDos(toDos) {
  // Tu código acá:
  return toDos.map(function(objetoElemento,  indice){
    return buildToDo(objetoElemento,  indice)
  })
...
function displayToDos() {
  ...
  let auxArreglo = buildToDos(toDoItems);
  for (let i = 0; i < auxArreglo.length;i++) {
    toDoContainer.appendChild(auxArreglo[i])
  }
